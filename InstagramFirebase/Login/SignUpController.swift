...
class SignUpController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        if let editetImage = info["UIImagePickerControllerEditetImage"] as? UIImage {
            plusPhotoButton.setImage(editetImage.withRenderingMode(.alwaysOriginal), for: .normal)
    ...
    @objc func handleSignUp() {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
            ...
            Storage.storage().reference().child("profile_image").child(filename).putData(uploadData, metadata: nil, completion: { (metadata, err) in
                ...
                if let err = err {
                    print("failed to upload profileimage to storage", err)
                    return
                }
                
                guard let profileImageUrl = metadata?.downloadURL()?.absoluteString else { return }
                
                print("sucessfully uploaded profileimage", profileImageUrl)
                ...
                Database.database().reference().child("users").updateChildValues(values, withCompletionBlock: { (err, ref) in
                    ...
                    print("sucessfully saved userinfo to db")
